...
Returns those countries' names in a vector
*/
std::vector< std::string > BTree::findOperation(double tarMean, std::string operation){
    //if the tree is empty, stop
    if (root == nullptr){
        std::cout << std::endl;
        //return empty vector
        std::vector< std::string > placeholdArr;
        return placeholdArr;
    }
    ...
    if (operation == "less"){
        ...
        return prevNode -> returnTargetMean(tarMean, operation);
    ...
    //if none are applicable, return empty array as a failsafe
    std::vector< std::string > placeholdArr;
    return placeholdArr;
}
