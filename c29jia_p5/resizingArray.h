...
class ResizingArray {
    private:
        ...
        int numValidValues; //number of non -1 values, incremented OUTSIDE the class (due to comparisons between T and ints)
        T *arr = new T[2]; //dynamic array of type T
    ...
    public:
        ...
        void appendVal(T value){
            ...
            if (numValues == 0){
                // std::cout << numValues << "you're in append :D" << std::endl;
                numValues++;
                arr[0] = value;
                return;
            }
            ...
            if (this -> isFull()){
                // std::cout << "you're in capacity double :D" << std::endl;
                this -> doubleCapacity();
            }
        ...
        void emptyArray(){
            ...
            //deallocate old array and set the temporary pointer to null pointer
            delete[] temp;
            temp = nullptr;
        }
        ...
        void incrementNumValidValues(){
            numValidValues++;
            // std::cout << "incrementing :D" << std::endl;
        }
        void incrementNumValues(){
            numValues++;
        }
        //decrementers
        void decrementNumValidValues(){
            numValidValues--;
        }
        void decrementNumValues(){
            numValues--;
        }
        //set values to 0
        void zeroNumValues(){
        ...
        void zeroNumValidValues(){
            numValidValues = 0;
        }
        //empties if T is a pointer for objects
        void emptyPointerArray(){
            //call the destructor for each object pointer in the array
            for (int i = 0; i < numValues; i++){
                T temp = arr[i];
                delete arr[i];
                temp = nullptr;
