...
class TimeSeries {
    ...
    public:
        ...
        //faulty because it doesn't take into account the -1 invalid datas
        //corner cases not yet considered
        //tested
        /*
        bool is_monotonicOLD(){
            //special cases for when there are 0, 1, or 2 values in the data list
            //if no values, no valid data
            ...
            //make a new validVal to compare to future values, set it to second val to avoid duplicating work.
            double validVal = firstVal;
            //If the first two values are increasing monotonic
            if (isIncreasing){
                //iterate through the array, ignoring invalid values
                for (int i = 0; i < dataList.getNumValues() - 1; i++){
                    //when you reach a valid value, compare it to validVal and check if it breaks monotonic (if validVal is greater)
                    if (dataList[i] >= 0){
                        if (validVal > dataList[i]){
                            //if yes return not monotonic and return
                            std::cout << "series is not monotonic" << std::endl;
                            return false;
                        }
                        //if not set validVal to the new valid value and keep going until the end of the array
                        validVal = dataList[i];
                    }
                }
            } else {
                //else if the first two values were decreasing monotonic, iterate through the array ingoring invalid values
                for (int i = 0; i < dataList.getNumValues() - 1; i++){
                    //when you reach a valid value, compare it to validVal and check if it breaks monotonic (if validVal is lesser)
                    if (dataList[i] >= 0){
                        if (validVal < dataList[i]){
                            //if yes return not monotonic and return
                            std::cout << "series is not monotonic" << std::endl;
            ...
            //if the array was no proven to be non monotonic then it must be monotonic
            std::cout << "series is monotonic" << std::endl;
            return true;
        }
        */
